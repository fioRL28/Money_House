...
// sin blur (ok para móvil)
const riseUp: Variants = {
  hidden: { y: 70, opacity: 0, scale: 0.99 },
  show: {
    y: 0,
    opacity: 1,
    scale: 1,
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        ...
        if (!armedRef.current && entry.isIntersecting && ratio >= 0.2) {
          armedRef.current = true;
          setVisible(true);
          return;
        }

        if (armedRef.current && !entry.isIntersecting) {
      ...
      >
        <div className="mx-auto max-w-[1320px] px-4 sm:px-6 lg:px-10 py-10 lg:py-14">
          ...
          <div className="mt-10 sm:mt-14 grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12 items-stretch">
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-6">
              <div className="animate-float shrink-0" style={{ animationDelay: "0.5s" }}>
                <Image
            ...
            {/* CTA clickable y accesible */}
            <Link
              href="/user/register"
              className="
                bg-[#1646C6] rounded-[28px] px-6 sm:px-8 py-8 sm:py-10 text-white
                shadow-xl flex flex-col sm:flex-row items-center justify-center
                gap-5 sm:gap-6 border-2 border-white/10
                transition-transform
                motion-safe:hover:scale-[1.03]
                focus:outline-none focus-visible:ring-2 focus-visible:ring-white/70
              "
              aria-label="Crear alerta: regístrate"
            >
              <motion.div
